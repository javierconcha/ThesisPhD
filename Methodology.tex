...
\node (AtmCorr) [process, below of=mask] {Atmospheric Correction};
\node (rrs) [io, below of=AtmCorr] {$R_{rs}$ \\Image};
\node (RetProc) [process, below of=rrs] {Retrieval Process};
\node (CPAmap) [io, below of=RetProc] {CPA \\maps};

\node (CDRImage) [io, left of=L8image, xshift=-2cm] {Landsat \\Reflectance \\Product};
\node (PIFmask) [process, below of=CDRImage] {PIF \\Mask};
\node (brightpx) [io, below of=PIFmask] {Bright\\Pixel};

\node (samples) [io, right of=L8image, xshift=2cm] {Water \\Samples};
\node (labmea) [process, below of=samples] {Lab \\Mea.};
\node (IOPs) [io, below of=labmea] {IOPs \\and \\Concent.};
\node (hydro) [process, below of=IOPs] {Hydrolight};
\node (dark) [iosmall, below of=hydro, xshift=-1cm] {Dark \\Pixel};
\node (LUT) [iosmall, below of=hydro, xshift=1cm] {LUT};


\node (AOPs) [io, right of=samples, xshift=2cm] {{\it in situ} \\AOPs \\($R_{rs}$)};
