...
One approach that was not tested in the research is using the results from the traditional \gls{elm}  or MoB-ELM \todo{MoB-ELM only?} algorithms as input to the standard NASA models and investigated whether the retrieval improves or not. On the other hand, the results from the \citep{Gordon:1994} algorithm could be used as input to the developed LUT-based inversion method and investigate if the results improve or not.
% -------------------------------------
\section{Future Work}
